...
class Form {
	...
	public static function load_texdomain() {
		load_textdomain( 'svbk-form', dirname( __DIR__ ) . '/languages/svbk-form-' . get_locale() . '.mo' );
	}
	...
	protected static function fieldError( $fieldAttr, $name = '' ) {
		return ( isset( $fieldAttr['error'] ) ? $fieldAttr['error'] : sprintf( __( 'Empty or invalid field [%s]', 'svbk-form' ), $name )  );
	}
